...
class Teacher extends Admin_Controller {
    ...
    */
    ...
    public function view() {
        $teacherID = htmlentities((string) escapeString($this->uri->segment(3)));
    ...
    private function teacherinfo($teacherinfo) {
        $this->data['profile'] = inicompute($teacherinfo) ? $teacherinfo : [];
    }

    private function documentInfo($teacherinfo) {
        if (inicompute($teacherinfo)) {
            $this->data['documents'] = $this->document_m->get_order_by_document(['usertypeID' => 2, 'userID' => $teacherinfo->teacherID]);
        } else {
            $this->data['documents'] = [];
        }
    }
    ...
    public function documentUpload() {
        ...
        if (permissionChecker('teacher_add') && permissionChecker('teacher_delete')) {
            ...
            $retArray['status'] = FALSE;
            $retArray['render'] = 'Permission Denied.';
            echo json_encode($retArray);
            exit;
        }
    }
    ...
    public function lol_username() {
        ...
        if ((int)$id !== 0) {
            ...
            $tables = ['student' => 'student', 'parents' => 'parents', 'teacher' => 'teacher', 'user' => 'user', 'systemadmin' => 'systemadmin'];
            $array = [];
            foreach ($tables as $table) {
                $user = $this->teacher_m->get_username($table, ["username" => $this->input->post('username'), "username !=" => $teacher_info->username]);
                if (inicompute($user)) {
    ...
    public function unique_email() {
        ...
        if ((int)$id !== 0) {
            ...
            $array = [];
            foreach ($tables as $table) {
                $user = $this->teacher_m->get_username($table, ["email" => $this->input->post('email'), 'username !=' => $teacher_info->username]);
                if (inicompute($user)) {
