...
class Online_exam extends Admin_Controller
{
    ...
    */
    ...
    public function get_payment_info() //done
    {
        ...
        if (permissionChecker('take_exam') && (!empty($course_slug))) {
            $courseDetails = $this->online_exam_m->get_single_course_detail(['slug' => $course_slug]);
           
            if (inicompute($courseDetails)) {
                $retArray['status']        = true;
                $retArray['payableamount'] = sprintf("%.2f", $courseDetails->cost);
            }
        }
